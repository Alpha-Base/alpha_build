...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^tuesday_") ; then
       CUSTOM_BUILD=$(echo -n $1 | sed -e 's/^tuesday_//g')
    else
       CUSTOM_BUILD=
    fi
    export CUSTOM_BUILD
...
function breakfast()
{
    ...
    unset LUNCH_MENU_CHOICES
    for f in `/bin/ls vendor/tuesday/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the tuesday model name
            lunch tuesday_$target-userdebug
        fi
    fi
    return $?
}
